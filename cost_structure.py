    ...
    def cm_auto(self) -> {(str, str): np.array}:
        if not self._cm_auto:
            ...
            for c_r, c_p in self.pairs:
                ...
                self._cm_auto[(c_r, c_p)] = confusion_matrix(self.manual_labels[(c_r, c_p)], h)
        return self._cm_auto

    def get_manual_costs(self, test_set_start: int, test_set_end: int) -> ({(str, str): float},):
        cost_manual = dict()
        cost_manual_ann = dict()
        cost_manual_misc = dict()
        for c_r, c_p in self.pairs:
            matrix_r, matrix_p = self.__manual_contingency_table(c_r, c_p)
            cost_ann = self.structure.relev * (test_set_end - test_set_start) + self.structure.priv * (matrix_r[0][0] + matrix_r[0][1])
            cost_misc = self.__manual_misclass_matrix(matrix_r, matrix_p, test_set_end - test_set_start)

...
class MineCore:
    ...
    def __init__(self, pairs: [(str, str)], prior_probabilities: {str: float}, posterior_probabilities: {str: float},
    ...
    def run(self, costs):
        ...
        for c_r, c_p in self.pairs:
            ...
            # ncr case
            risk_h_2_ncr = self.get_h_risk_h(np.zeros_like(c_r_probs[:, 1]), c_p_probs[:, 1], cost_matrix, need_risk=True)[1]
    ...
    def run_plusplus(self, costs):
        """
        Run the three phases of MineCore Plus Plus (eg. uses alpha values)
        :type costs: cost_structure.Costs
        :returns tau_rs, tau_ps, confusion matrix for phase 2 and phase 3 (cm_2 and cm_3)
        """
        cost_matrix = costs.cost_matrix
    ...
    def custom_run(self, costs):
        ...
        for c_r, c_p in self.pairs:
            ...
            # phase two - responsiveness
            cr_human_probs = np.where(c_r_probs[:, 1] > 0.5, 1, 0)

            e_risk_h_2 = self.get_h_risk_h(cr_human_probs, c_p_probs[:, 1], cost_matrix, need_risk=True)[1]
            ...
            # phase three - privilege
            # cp case
            cp_human_probs = np.where(c_p_probs[:, 1] > 0.5, 1, 0)
            e_risk_h_3 = self.get_h_risk_h(c_r_probs_2, cp_human_probs, cost_matrix, need_risk=True)[1]
